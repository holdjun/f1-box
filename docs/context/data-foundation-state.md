...
- 2026-08-12 data-sync 修复（8-09 起连续 4 天挂）：tag 解析取第一跳重定向（原 -L 跟到 CDN 末跳无 /download/ 段，tag 恒空→门禁失效、release.json 天天被空 tag 覆盖）；previous 解析改为落盘再读（wrangler 横幅走 stdout，管道 JSON.parse 恒崩→previous=none→每天误判 changed=true 触发全量重导）；tag 为空直接中止；release.json 降为成功标记，D1 导入成功后才写入（失败下次自动重试）。云端 release.json 已手工修复为 v2026.11.0（D1 于 8-08 由该版本导入，门禁恢复短路）。遗留：CI token 缺 D1 写权限（/import 端点 code 10000），下次上游发新版时 D1 步骤会挂，需用户在 Cloudflare 面板给 CLOUDFLARE_API_TOKEN 加 Workers D1 Edit 权限。
