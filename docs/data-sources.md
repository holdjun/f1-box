...
| 未来赛程与最近赛程变更 | 经比赛身份核对、更新时间明确的 FastF1 schedule 优先，缺失回落 f1db | 两个来源的抓取时间不能直接代表内容新旧，改期需显式处理 |
...
每批记录来源、源版本或对象哈希、解析器版本、抓取时间、结果状态。抓取时间只表示何时获取，不表示上游何时修订。f1db 到货后比较完整场次再接管；其缺失场次继续保留 FastF1。差异留在同步摘要中，不自动修改历史积分去“配平”。
...
5. 目标是一小时内提供赛后结果，但 GitHub Actions 排队和上游可用性都可能延迟；首个功能测量实际延迟后再决定是否需要常驻任务执行器。

PR #32 的天气仍是简化版本：每天重试当季空缺，已有行跳过，历史回填手动执行。未来可复查部分数据的策略需要额外同步状态，不能声称现有 `--have` 已支持。
...
1. 合并 PR #32：历史时刻补全 + 可选天气，真实回填后验收。
