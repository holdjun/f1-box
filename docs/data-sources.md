...
- 站点表属于我们：当前是 `session_time`、`session_weather`、`weather_sync_state`；快速结果与车手映射等表等相应功能落地时再建。
...
4. 天气同步状态区分 `success`、`no_data`、`mismatch`、`failed`、`exhausted`。无数据与网络失败不可混为一谈；失败设置退避，达到上限后保留错误摘要，不覆盖已有快照。
5. 目标是一小时内提供赛后结果，但任务调度和上游可用性都可能延迟；首个功能测量实际延迟，不把定时触发视为准点完成的保证。

天气已按该原则落地：`apps/weather-sync` 的 Cloudflare Worker 定时触发 Workflows，按年调用预装依赖的 FastF1 Container；D1 保存摘要与逐 session 状态，成功写入后按 `f1db` 缓存标签刷新页面。GitHub Actions 只保留 f1db 导入、CI 与部署，不再承担天气取数。实现细节与验收记录见 [天气同步执行环境](decisions/weather-sync-cloudflare.md)。
...
1. 完成 PR #32：Cloudflare 采集、天气同步、历史回填和缓存更新已实现，验收后合并。
