...
Architecture: 纯 Astro SSR + 原生 TS island；后端 handler / agent / tools / 纯函数模块分层，全部依赖注入可单测；模型调用用 @cloudflare/ai-utils 的 runWithTools（strictValidation + streamFinalResponse），工具执行期间的 status 事件由工具闭包直接写入自建 SSE 流。

Tech Stack: Astro 7（@astrojs/cloudflare）、Cloudflare Workers AI（@cf/zai-org/glm-4.7-flash）、@cloudflare/ai-utils、D1（f1db）、Workers rate limiting binding、Vitest、Playwright。
...
describe("resolveDriver", () => {
  it("binds the query as a single parameter and reports ambiguity", async () => {
    ...
    expect(sql).toMatch(/^SELECT id, name FROM driver/);
...
### Task 7: Agent（提示词 + runWithTools + SSE 流组装）

Files:
- Modify: apps/web/package.json（依赖 @cloudflare/ai-utils）
...
- Produces: `ASK_MODEL_ID = "@cf/zai-org/glm-4.7-flash"`；`buildSystemPrompt(entries: KnowledgeEntry[]): string`；`decodeFinalChunk(chunk: string): string`；`RunToolsFn`；`runAgent(options: { ai: Ai; db: AskDatabase; messages: AskMessage[]; runTools?: RunToolsFn }): ReadableStream<Uint8Array>`。Task 8 消费。

- [ ] Step 1: 安装依赖

```bash
pnpm --filter @f1-box/web add @cloudflare/ai-utils
```
  ...
  return new ReadableStream<Uint8Array>({
    async start(controller) {
      ...
      try {
        ...
        const finalStream = (await runTools(ai, ASK_MODEL_ID, {
          messages: [
          ...
          tools,
          temperature: 0.3,
          // 输出上限；参数名以绑定通道 schema 实测为准（Task 8 Step 7 冒烟时确认，
          // 若报 schema 错误则换成 max_completion_tokens）
          max_tokens: 1024,
        }, {
          strictValidation: true,
          streamFinalResponse: true,
...
```bash
git add apps/web/package.json apps/web/src/lib/ask/agent.ts apps/web/tests/ask-agent.test.ts
git commit -m "feat: add ask agent with runWithTools and sse stream"
```
