...
- Produces: `ASK_MODEL_ID = "@cf/zai-org/glm-4.7-flash"`；`buildSystemPrompt(entries: KnowledgeEntry[]): string`；`createFinalChunkDecoder(): { push(chunk: string): string; flush(): string }`；`RunToolsFn`；`runAgent(options: { ai: Ai; db: AskDatabase; messages: AskMessage[]; runTools?: RunToolsFn }): ReadableStream<Uint8Array>`。Task 8 消费。
