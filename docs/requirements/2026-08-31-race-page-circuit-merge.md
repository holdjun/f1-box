...
- 赛道 SVG 保持现状（优先注解版地图，无注解回落 f1db 轮廓），不再是链接；与信息卡同面板语言（边框、底色、圆角一致）相邻展示（桌面并排、移动端堆叠）。注解地图在图下渲染图例（Sector 三色、弯号角标，DRS 仅在有数据时出现）；无注解回落不渲染图例。
...
- `/circuits` 与 `/circuits/:id` 页面移除，不保留任何 circuits 专属重定向：`/circuits/:id` 自然 404；裸 `/circuits` 与任意未知单段路径同口径，由既有 `[year]` 路由兜底 302 回首页。
- 车手徽标统一用官方三字码（如 RUS），不再用名字首字母：分站各结果表、车手/车队积分榜、比赛列表冠军列、车手目录卡与详情页无车号回落位一致。
...
- `/circuits/melbourne` 返回 404；裸 `/circuits` 由既有 `[year]` 路由兜底 302 回首页（与 `/banana` 等任意未知单段路径同口径），无 circuits 专属重定向。
