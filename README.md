...
技术路线：Astro（Cloudflare Workers）+ Svelte 5 islands（仅交互组件）+ Tailwind CSS v4（双层令牌、深/亮双主题）+ f1db 数据经周同步进 D1 + FastF1 天气经 Cloudflare Workflows 采集。

## 目录结构

- `apps/web`：Astro 站点，部署为 Cloudflare Worker。本地开发读 fixture，生产读 D1。
- `apps/weather-sync`：Cloudflare Worker + Workflows + Container，负责 FastF1 天气采集与 D1 写入。
...
f1db（CC-BY-4.0）→ `data-sync.yml` 按上游发布节奏轮询 release tag（周日晚到周二每 2 小时、其余每天一次），有变化时全量导入 D1 → Astro Worker 读 D1。FastF1 天气 → `apps/weather-sync` 的 Cloudflare Workflows 调 Container 采集 → 写入站点扩展表并刷新缓存。访客请求不直接访问上游数据源。
...
5. weather-sync Worker 每 30 分钟检查当季已结束且未完成的场次；历史回填通过其认证 API 手动触发。
